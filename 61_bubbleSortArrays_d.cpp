// To write a program ..Bubble Sort in decreasing order of an Array.
...
void bubbleSort_d(int sorted_array[],int n){
    for (int i=0; i<n-1; i++){                  
        ...
        for (int j=0; j<n-1-i; j++){
            ...
            //Agar right wala bada hai to swap, aise smallest will be send to end.
...
int main(){
    ...
    bubbleSort_d(sorted_array,n);    //Here we have just called the function and not took anything in return as array which will be sent will change for permanent as it works with address.
